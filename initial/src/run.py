...
sys.path.append('./main/pymjc/utils/')
sys.path.append('./main/pymjc/astgen/')
sys.path.append('./main/pymjc/checker/')
sys.path.append('./main/pymjc/codegen/')

ANTLR_JAR = './antlr-4.9.2-complete.jar'
TARGET_DIR = '../target'
GENERATE_DIR = '' if sys.platform.startswith('win') else 'main/pymjc/parser'
...
def main(argv):
    if len(argv) < 1:
        ...
        print("hello")
        subprocess.run(["java", "-jar", ANTLR_JAR, "-Dlanguage=Python3", "-o",
                       "../target", "-no-listener", "-visitor", "main/pymjc/parser/PYMJC.g4"])
    elif argv[0] == 'clean':
        subprocess.run(["rm", "-rf", TARGET_DIR + "/*"])
    elif argv[0] == 'test':
        if not os.path.isdir(TARGET_DIR + "/" + GENERATE_DIR):
            subprocess.run(["java", "-jar", ANTLR_JAR, "-o", GENERATE_DIR,
                           "-no-listener", "-visitor", "main/pymjc/parser/PYMJC.g4"])
